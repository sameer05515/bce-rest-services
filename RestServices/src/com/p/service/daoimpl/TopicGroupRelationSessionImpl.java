...
public class TopicGroupRelationSessionImpl implements TopicGroupRelationSession {
	...
	@Override
	// topicIdList
	public List<TopicGroupRelation> getTopicGroupRelationForTopic(List<Integer> lobIdList) throws RestServiceException {
		...
		// if valid.
		if (lobIdList == null || lobIdList.size() == 0) {
		...
		try {
			...
			Query hqlQuery = session.createQuery("from TopicGroupRelation where topics.id in (:lobIdList)");
